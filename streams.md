...
### Enable Built-in IP Address-based Filters
...
will undergo probabilistic analysis by our [VLA™ machine learning model](vla.md), if available in your plan.

**It is strongly recommended to enable this setting unless you have a compelling reason not to.**
...
IPv4 addresses** that are considered "paranoid", that is, with higher false positives chances, but at the same time
providing equally higher chance of busting moderators and other malicious parties.  In particular, these lists block
questionable ISPs that provide mixed Internet access, hosting, and/or IP transit services.

**We recommend to enable this mode when working with TikTok!**  In Google Ads, this setting is advisable at
campaign start and may be lifted after a few days of running.
...
that first 10--15 clicks in each campaign in your ad network belong to moderators and malware checkers, then you may
...
Please note that this feature works only when the stream is in Filter mode.  This may be convenient if you launch many
ad campaigns and cannot or do not want to watch every one of them to flip the stream mode from On Review to Filter.
...
**We strongly recommend to set it to 1 when working with Google Ads and TikTok!**

## IP Address Filtering

Adspect supports traffic filtering by user-defined lists of IP addresses, IP address ranges, CIDR prefixes,
and/or [autonomous system numbers (ASN)](https://en.wikipedia.org/wiki/Autonomous_system_(Internet)).
There are two lists per each stream: the blacklist and the whitelist.
...
and lower gradually if you experience high traffic losses.
...
* **Black**: a visitor will be blocked only if their IP address or ASN is in the blacklist and is not in the whitelist.
  As such, the whitelist may be used to override the blacklist.  Whitelisted addresses are also exempt from all internal
  IP/ASN blocklists of Adspect.  This is the default mode.
...
* **Special**: a visitor will be blocked if their IP address or ASN is in the blacklist, and allowed through
  unconditionally if their IP address or ASN is in the whitelist.
...
is not required, but may be useful if you want to track individual clicks, conversions, spend, revenue,
and per-source statistics in the [Reporting](reporting.md) section of Adspect.
...
This setting works similarly to the user agent blacklist described above, but deals with [HTTP referer](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Referer)
